        ...
        <ActionSheet id={modalId} animated={true} onBeforeShow={(data) => { setData(data); }}>
            <View style={{ height: '75%' }}>
                ...
                <View style={{paddingHorizontal: 30}}>
                    <TouchableOpacity style={{backgroundColor: '#147DF5', }} underlayColor="#08386F" onPress={() => {
                        SheetManager.show(reminderModalId, data);
                     }} >
                        <View style={styles.customerModal.reminderBtn}>
                            <MaterialCommunityIcons name="clock-time-eight-outline" size={24} color="#fff" />
                            <Text style={styles.customerModal.reminderBtnText}>Send a Reminder</Text>
                        </View>
                    </TouchableOpacity>
                </View>
            </View>
...
function CustomerReminderModal() {
    ...
    return (
        <ActionSheet id={reminderModalId} containerStyle={{ justifyContent: 'center' }} onClose={() => { SheetManager.hide(modalId) }} onBeforeShow={(data) => { setData(data); }}>
            <View style={{ height: '75%' }}>
                <View style={[styles.customerModal.dash, {top: -75}]} />
                <View style={{flex: 1, justifyContent: 'center'}}>
                    <View style={{ padding: 20, paddingHorizontal: 30 }}>
                        <View style={{ marginBottom: 15, alignItems: 'center'}}>
                            <MaterialCommunityIcons name="check-circle" size={64} color="#38B000" />
                            <Text style={{fontSize: 18, marginVertical: 15, textAlign: 'center', fontFamily: 'Manrope_700Bold'}}>Reminder sent successfully</Text>
                            <Text style={{fontSize: 14, textAlign: 'center', fontFamily: 'Manrope_400Regular'}}>You've successfully sent a reminder to {<Text style={{fontWeight: 'bold'}}>{data.name}</Text>} he will get a notification to place order.</Text>
                        </View>
                    </View>
                    <View style={{paddingHorizontal: 30}}>
                        <TouchableHighlight style={{backgroundColor: '#147DF5', }} underlayColor="#08386F" onPress={() => {}} >
                            <View style={styles.customerModal.reminderBtn}>
                                <Text style={styles.customerModal.reminderBtnText}>Back Home</Text>
                            </View>
                        </TouchableHighlight>
                    </View>
                </View>
...
function showReminderSuccessDetails(data) {
    SheetManager.show(reminderModalId, {...data});
}
            ...
            <View style={{width: '100%', height: '100%'}}>
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <MaterialCommunityIcons name="magnify" size={24} color="#979797" style={{
                            marginRight: 10
                        }} />
                        <TextInput placeholder='Search' style={{color: '#979797', fontFamily: 'Manrope_400Regular', flex: 1}} />
                    </View>
                    <View style={{
                        flexDirection: 'row',
                        alignItems: 'center',
                        ...
                        <MaterialCommunityIcons name="filter-outline" size={24} color="#21334F" />
                    </View>
                </View>

                <View style={{ width: '100%', padding: 20 }}>
                    <Text style={{ fontFamily: 'Manrope_700Bold', fontSize: 18 }}>Favourite Customers</Text>
                </View>
                ...
                <View style={{ paddingHorizontal: 20, flex: 1 }}>
                    <FlatList 
                        ...
                        renderItem={({ item }) => {
                            return (
                                <FavouriteCustomer name={item.name} image={item.image} location={item.location} onPress={() => { showDetails(item) }} />
                            );
                        }}
